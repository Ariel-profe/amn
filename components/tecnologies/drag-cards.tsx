...
export const DragCards = () => {
  return (
    <section className="cursor-grab relative grid h-[80vh] lg:min-h-screen border rounded-md place-content-center overflow-hidden mt-10">
      <img src="/logo.svg" alt="amn logo" className="w-[300px] lg:w-[500px]" />
      <Cards />
    </section>
  );
};
...
const Card = ({ containerRef, src, alt, top, left, rotate, className }:{ containerRef:MutableRefObject<null>, src:string, alt:string, top:string, left:string, rotate:string, className:string }) => {
  ...
  return (
    <motion.img
      ...
      className={twMerge(
        "drag-elements absolute w-48 bg-neutral-200 p-1 pb-4 focus:cursor-grabbing",
        className
      )}
      src={src}
      alt={alt}
